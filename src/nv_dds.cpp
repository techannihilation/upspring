...
//#  include <GL/glx.h>
#define GET_EXT_POINTER(name, type) name = (type)glXGetProcAddressARB((const GLubyte*)#name)
#else
...
///////////////////////////////////////////////////////////////////////////////
// default constructor
CDDSImage::CDDSImage() : m_format(0), m_components(0), m_type(TextureNone), m_valid(false) {}

CDDSImage::~CDDSImage() {}

void CDDSImage::create_textureFlat(unsigned int format, unsigned int components,
                                   const CTexture& baseImage) {
  assert(format != 0);
  assert(components != 0);
...
void CDDSImage::create_texture3D(unsigned int format, unsigned int components,
                                 const CTexture& baseImage) {
  assert(format != 0);
  assert(components != 0);
  assert(baseImage.get_depth() > 1);
...
bool CDDSImage::load(std::string filename, bool flipImage) {
  ...
  if (ddsh.dwCaps2 & DDSF_CUBEMAP) m_type = TextureCubemap;

  // check if image is a volume texture
  if ((ddsh.dwCaps2 & DDSF_VOLUME) && (ddsh.dwDepth > 0)) m_type = Texture3D;
...
bool CDDSImage::save(std::string filename, bool flipImage) {
  ...
  if (is_compressed()) {
    ...
    if (m_format == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ddsh.ddspf.dwFourCC = FOURCC_DXT1;
    if (m_format == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT) ddsh.ddspf.dwFourCC = FOURCC_DXT3;
    if (m_format == GL_COMPRESSED_RGBA_S3TC_DXT5_EXT) ddsh.ddspf.dwFourCC = FOURCC_DXT5;
  } else {
  ...
  if (get_num_mipmaps() > 0) ddsh.dwCaps1 |= DDSF_COMPLEX | DDSF_MIPMAP;
...
bool CDDSImage::upload_texture1D() {
  ...
  if (is_compressed()) {
    ...
    if (glCompressedTexImage1DARB == NULL) {
      GET_EXT_POINTER(glCompressedTexImage1DARB, PFNGLCOMPRESSEDTEXIMAGE1DARBPROC);
    }

    if (glCompressedTexImage1DARB == NULL) return false;

    glCompressedTexImage1DARB(GL_TEXTURE_1D, 0, m_format, baseImage.get_width(), 0,
                              baseImage.get_size(), baseImage);

    // load all mipmaps
    for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++) {
      const CSurface& mipmap = baseImage.get_mipmap(i);
      glCompressedTexImage1DARB(GL_TEXTURE_1D, i + 1, m_format, mipmap.get_width(), 0,
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, m_components, baseImage.get_width(), 0, m_format,
    ...
    for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++) {
      ...
      glTexImage1D(GL_TEXTURE_1D, i + 1, m_components, mipmap.get_width(), 0, m_format,
                   GL_UNSIGNED_BYTE, mipmap);
    }

    if (alignment != -1) glPixelStorei(GL_UNPACK_ALIGNMENT, alignment);
  }
...
bool CDDSImage::upload_texture2D(unsigned int imageIndex, int target) const {
  ...
  if (is_compressed()) {
    glCompressedTexImage2D(target, 0, m_format, image.get_width(), image.get_height(), 0,
                           image.get_size(), image);

    // load all mipmaps
    for (unsigned int i = 0; i < image.get_num_mipmaps(); i++) {
      const CSurface& mipmap = image.get_mipmap(i);
      glCompressedTexImage2D(target, i + 1, m_format, mipmap.get_width(), mipmap.get_height(), 0,
                             mipmap.get_size(), mipmap);
    }
  } else {
    GLint alignment = -1;
    ...
    glTexImage2D(target, 0, BASE_INT_FORMATS[m_components], image.get_width(), image.get_height(),
                 0, m_format, GL_UNSIGNED_BYTE, image);
...
bool CDDSImage::upload_texture3D() {
  ...
  if (is_compressed()) {
    ...
    if (glCompressedTexImage3DARB == NULL) {
      GET_EXT_POINTER(glCompressedTexImage3DARB, PFNGLCOMPRESSEDTEXIMAGE3DARBPROC);
    }

    if (glCompressedTexImage3DARB == NULL) return false;

    glCompressedTexImage3DARB(GL_TEXTURE_3D, 0, m_format, baseImage.get_width(),
                              baseImage.get_height(), baseImage.get_depth(), 0,
                              baseImage.get_size(), baseImage);
    ...
    for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++) {
      ...
      glCompressedTexImage3DARB(GL_TEXTURE_3D, i + 1, m_format, mipmap.get_width(),
                                mipmap.get_height(), mipmap.get_depth(), 0, mipmap.get_size(),
                                mipmap);
    }
  } else {
    // retrieve function pointer if needed
    ...
    if (glTexImage3D == NULL) return false;
...
bool CDDSImage::upload_textureRectangle() { return upload_texture2D(0, GL_TEXTURE_RECTANGLE_NV); }
...
bool CDDSImage::upload_textureCubemap() {
  ...
  for (unsigned int n = 0; n < 6; n++) {
    ...
    target = GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + n;
    if (!upload_texture2D(n, target)) return false;
  }
...
inline void CDDSImage::swap_endian(void* val) {
  ...
  *ival = ((*ival >> 24) & 0x000000ff) | ((*ival >> 8) & 0x0000ff00) | ((*ival << 8) & 0x00ff0000) |
          ((*ival << 24) & 0xff000000);
#else
  (void)val;
#endif
...
CTexture::CTexture(unsigned int w, unsigned int h, unsigned int d, unsigned int imgsize,
...
///////////////////////////////////////////////////////////////////////////////
// copy constructor
CTexture::CTexture(const CTexture& copy) : CSurface(copy) {
  for (unsigned int i = 0; i < copy.get_num_mipmaps(); i++) m_mipmaps.push_back(copy.get_mipmap(i));
}
...
CTexture& CTexture::operator=(const CTexture& rhs) {
  if (this != &rhs) {
    ...
    m_mipmaps.clear();
    for (unsigned int i = 0; i < rhs.get_num_mipmaps(); i++) m_mipmaps.push_back(rhs.get_mipmap(i));
  }

  return *this;
}

void CTexture::create(unsigned int w, unsigned int h, unsigned int d, unsigned int imgsize,
                      const unsigned char* pixels) {
...
///////////////////////////////////////////////////////////////////////////////
// default constructor
CSurface::CSurface() : m_width(0), m_height(0), m_depth(0), m_size(0), m_pixels(NULL) {}

///////////////////////////////////////////////////////////////////////////////
// creates an empty image
CSurface::CSurface(unsigned int w, unsigned int h, unsigned int d, unsigned int imgsize,
                   const unsigned char* pixels)
...
///////////////////////////////////////////////////////////////////////////////
// creates an empty image
void CSurface::create(unsigned int w, unsigned int h, unsigned int d, unsigned int imgsize,
                      const unsigned char* pixels) {
  assert(w != 0);
  assert(h != 0);
  assert(d != 0);
