 ...
 * for (uint fid = 0; fid < NumFiles(); ++fid) {
 * 	//stuff
 * }
 */
class IArchive {
 ...
 public:
   ...
   * archive.
   */
  inline bool IsFileId(uint fileId) const { return (fileId < NumFiles()); }
  /**
   ...
   */
  uint FindFile(const std::string& filePath) const;
  /**
   ...
   * @see GetFile(uint fid, std::vector<boost::uint8_t>& buffer)
   */
  virtual bool GetFile(uint fid, std::vector<std::uint8_t>& buffer) = 0;
  /**
   ...
   */
  virtual void FileInfo(uint fid, std::string& name, int& size, int& mode) const = 0;
  /**
   ...
   */
  //	virtual bool HasLowReadingCost(uint fid) const;
 ...
 protected:
  ...
  std::unordered_map<std::string, uint> lcNameIndex;
