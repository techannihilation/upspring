...
class Vector3 {
 public:
  ...
  // copy to float[3]
  void copy(float* t) const {
    t[0] = x;
    t[1] = y;
    t[2] = z;
  }
  void copy(Vector3* t) const {
    t->x = x;
    t->y = y;
    t->z = z;
  }
...
class Plane {
 public:
  float a, b, c, d;
  Plane() { a = b = c = d = 0; }
  Plane(float ta, float tb, float tc, float td) {
    a = ta;
    b = tb;
    c = tc;
    d = td;
  }
  Plane(Vector3 dir, float dis) {
    a = dir.x;
    b = dir.y;
    c = dir.z;
    d = dis;
...
class Matrix {
 public:
  ...
  void copy(float* d) {
    for (int a = 0; a < 16; a++) d[a] = m[a];
  }
  void copy(Matrix* d) {
    for (int a = 0; a < 16; a++) d->m[a] = m[a];
  }
  ...
  // column access
  void getcx(Vector3& v) const {
    v.x = m[0];
    v.y = m[4];
    v.z = m[8];
  }
  void getcy(Vector3& v) const {
    v.x = m[1];
    v.y = m[5];
    v.z = m[9];
  }
  void getcz(Vector3& v) const {
    v.x = m[2];
    v.y = m[6];
    v.z = m[10];
  }
  void getcw(Vector3& v) const {
    v.x = m[3];
    v.y = m[7];
    v.z = m[11];
  }
  void setcx(const Vector3& v) {
    m[0] = v.x;
    m[4] = v.y;
    m[8] = v.z;
  }
  void setcy(const Vector3& v) {
    m[1] = v.x;
    m[5] = v.y;
    m[9] = v.z;
  }
  void setcz(const Vector3& v) {
    m[2] = v.x;
    m[6] = v.y;
    m[10] = v.z;
  }
  void setcw(const Vector3& v) {
    m[3] = v.x;
    m[7] = v.y;
    m[11] = v.z;
  }
...
class Quaternion {
 public:
  ...
  enum SlerpType { qshort, qlong, qcw, qccw, quser };

  void identity() {
    x = y = z = 0.0f;
    w = 1.0f;
  }
  void copy(Quaternion* p) const {
    p->x = x;
    p->y = y;
    p->z = z;
    p->w = w;
  ...
  void conjugate(Quaternion* dest) const {
    dest->x = -x;
    dest->y = y;
    dest->z = -z;
    dest->w = w;
