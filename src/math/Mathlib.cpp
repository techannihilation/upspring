...
static inline float Calc3x3Determinant (float m[9])
{
        ...
        return m [0] * (m[4]*m[8] - m[5]*m[7]) - m[1] * (m[3]*m[8]-m[5]*m[6]) + m[2] *
(m[3]*m[7]-m[6]*m[4]);
}
...
void Quaternion::slerp(const Quaternion* to, float t, Quaternion* out, SlerpType how) const {
  ...
  if (((how == qshort) && (cos_angle < 0.0)) || ((how == qlong) && (cos_angle > 0.0)) ||
      ((how == qcw) && (from->w > to->w)) || ((how == qccw) && (from->w < to->w))) {
    cos_angle = -cos_angle;
    to2.w = -to->w;
    to2.x = -to->x;
    to2.y = -to->y;
