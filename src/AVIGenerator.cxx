...
HRESULT CAVIGenerator::InitEngine() {
  ...
  hr = AVIFileOpen(
      &m_pAVIFile,           // Address to contain the new file interface pointer
  ...
  if (hr != AVIERR_OK) {
    ...
    switch (hr) {
      case AVIERR_BADFORMAT:
        m_sError +=
            _T("The file couldn't be read, indicating a corrupt file or an unrecognized format.");
        break;
      case AVIERR_MEMORY:
      ...
      case REGDB_E_CLASSNOTREG:
        m_sError +=
            _T("According to the registry, the type of file specified in AVIFileOpen does not ")
            _T("have a handler to process it");
        break;
    }
  ...
  strHdr.dwSuggestedBufferSize =
      m_bih.biSizeImage;          // Recommended buffer size, in bytes, for the stream.
  SetRect(&strHdr.rcFrame, 0, 0,  // rectangle for stream
          (int)m_bih.biWidth, (int)m_bih.biHeight);
  ...
  if (hr != AVIERR_OK) {
    ...
    switch (hr) {
      ...
      case AVIERR_UNSUPPORTED:
        m_sError +=
            _T("Compression is not supported for this type of data. This error might be returned ")
            _T("if you try to compress data that is not audio or video.");
        break;
    }
