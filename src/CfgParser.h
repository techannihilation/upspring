...
#define CFG_LOADN(cfg, val) (cfg).LoadNVal(#val, val)
  ...
  std::string& Load(const char* name, std::string& val) {
    const char* r = GetLiteral(name);
    if (r) val = r;
    return val;
  }
  CfgValue*& Load(const char* name, CfgValue*& val) {
    val = GetValue(name);
    return val;
  }
  bool& Load(const char* name, bool& val) {
    val = GetNumeric(name) != 0.0f;
    return val;
  }
  template <typename T>
  T& LoadNVal(const char* name, T& val) {
    val = (T)GetNumeric(name);
    return val;
  }
};
