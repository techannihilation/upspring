...
ImgFormat::ImgFormat(Type t) {
  for (int a = 0; a < 4; a++) mask[a] = shift[a] = loss[a] = 0;
  bytesPerPixel = 0;
  type = t;
...
void ImgFormat::CalcMask() {
  for (int a = 0; a < 4; a++) mask[a] = (0xff >> loss[a]) << shift[a];
}
...
Image::Image(int _w, int _h, const ImgFormat& fmt) { Alloc(_w, _h, fmt); }
...
void Image::FillColor(uint color) {
  uint c = format.Make((color & 0xff000000) >> 24, (color & 0xff0000) >> 16, (color & 0xff00) >> 8,
                       (color & 0xff) >> 0);
  memset(data, c, w * h);
}
...
void Image::SetPixel(int x, int y, uint src) {
  uint c = format.Make((src & 0xff000000) >> 24, (src & 0xff0000) >> 16, (src & 0xff00) >> 8,
                       (src & 0xff) >> 0);
...
void Image::LoadFromMemory(void* buf, int len) {
  ...
  if (!img) throw content_error(IMG_GetError());
...
void Image::LoadGrayscale(void* buf, int len) {
  ...
  if (bpp != 8) throw content_error(SPrintf("Grayscale image should be 8 bit, instead of %d", bpp));
...
void Image::DeleteIL(uint id) { iluDeleteImage(id); }
...
void Image::FromIL(uint id) {
  ...
  if (bpp == 4) {
    format = ImgFormat(ImgFormat::RGBA);
    Alloc(w, h, format);
    ilCopyPixels(0, 0, 0, w, h, 1, IL_RGBA, IL_UNSIGNED_BYTE, data);
  } else if (bpp == 3) {
    format = ImgFormat(ImgFormat::RGB);
    Alloc(w, h, format);
...
void Image::Load(const char* file, bool /*IsGrayscale*/) {
  ...
  if (!f) throw content_error("Failed to open file " + std::string(file));
...
void Image::SaveTGA(const char* file) {
  ...
  if (!pFile) throw std::runtime_error(SPrintf("Failed to open file '%s' for writing", file));
...
void Image::Convert(Image* dst) {
  ...
  if (!dst->data) dst->Alloc(w, h, dst->format);
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      for (a = 0; a < int(df->bytesPerPixel); a++) (*dstBytePos)[a] = 0;
