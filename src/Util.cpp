...
void AddTrailingSlash(std::string& tld) {
  if (tld.rfind('/') != tld.length() - 1 && tld.rfind('\\') != tld.length() - 1) tld.push_back('/');
}
...
void Logger::Realloc() {
  ...
  if (cb) {
    for (int a = 0; a < numcb; a++) nc[a] = cb[a];
    delete[] cb;
  }
...
void Logger::RemoveCallback(CallbackProc proc, void* data) {
  for (int a = 0; a < numcb; a++)
    if (cb[a].proc == proc && cb[a].user_data == data) {
      if (a != numcb - 1) std::swap(cb[a], cb[numcb - 1]);
      numcb--;
    }
}
...
void Logger::Trace(LogNotifyLevel lev, const char* fmt, ...) {
  ...
  if (!(lev & filter) && lev != NL_NoTag) return;
  ...
  for (int a = 0; a < numcb; a++) cb[a].proc(lev, buf, cb[a].user_data);
...
void Logger::Print(const char* fmt, ...) {
  ...
  for (int a = 0; a < numcb; a++) cb[a].proc(NL_NoTag, buf, cb[a].user_data);
...
void InputBuffer::ShowLocation() const { logger.Trace(NL_Debug, Location().c_str()); }
...
bool InputBuffer::CompareIdent(const char* str) const {
  ...
  while (str[i] && i + pos < len) {
    if (str[i] != data[pos + i]) return false;
...
bool InputBuffer::SkipWhitespace() {
  ...
  for (; !end(); next()) {
    if (get() == ' ' || get() == '\t' || get() == '\r' || get() == '\f' || get() == 'v') continue;
    if (get() == '\n') {
      line++;
      continue;
    }
...
void InputBuffer::SkipKeyword(const char* kw) {
  ...
  if (kw[a]) throw content_error(SPrintf("%s: Expecting keyword %s\n", Location().c_str(), kw));
}
...
std::string InputBuffer::ParseIdent() {
  ...
  if (isalnum(get())) {
    name += get();
    for (next(); isalnum(get()) || get() == '_' || get() == '-'; next()) name += get();
