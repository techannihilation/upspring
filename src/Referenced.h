...
class Referenced;
struct WeakPtrBase {
  // _ Prefixed items should not be used by external classes, except for the Referenced and
  // WeakPtr<T>::Type class
...
class Referenced {
 public:
  ...
  virtual ~Referenced() {
    for (WeakPtrBase* p = weakPtrList; p; p = p->_next) p->_object = 0;
  }
...
WeakPtrBase::~WeakPtrBase() { _Unlink(); }
...
class WeakPtr : public WeakPtrBase {
 public:
  WeakPtr() {}
  explicit WeakPtr(T* ptr) { _Link(ptr); }
  void Set(T* ptr) {
    _Unlink();
    _Link(ptr);
  }
