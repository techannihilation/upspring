...
class SpringModelPiece:
    def __init__(self, sceneObject):
        ...
        self.childPieces = []
        self.isRoot = self.parentPieceBO is None
    ...
    def SetParentPiece(self, p):
        self.parentPiece = p

    def GetparentPieceBO(self):
        return self.parentPieceBO

    def GetChildPieces(self):
        return self.childPieces

    def AddChildPiece(self, p):
        self.childPieces += [p]

    ## local offsets are wrt. the parent-piece
    def GetLocalOffsetX(self):
        return self.loffsetx

    def GetLocalOffsetY(self):
        return self.loffsety

    def GetLocalOffsetZ(self):
        return self.loffsetz

    ## global offsets are wrt. the root-piece
    def GetGlobalOffsetX(self):
        return self.goffsetx

    def GetGlobalOffsetY(self):
        return self.goffsety

    def GetGlobalOffsetZ(self):
        return self.goffsetz

    def GetLocalOffsetsStr(self):
        return "{%.2f, %.2f, %.2f}" % (self.loffsetx, self.loffsety, self.loffsetz)

    def GetGlobalOffsetsStr(self):
        return "{%.2f, %.2f, %.2f}" % (self.goffsetx, self.goffsety, self.goffsetz)

    def CalculateGlobalOffsets(self, modelMidPos):
        if not self.isRoot:
            assert self.parentPiece is not None
...
class SpringModel:
    ...
    def SetPieceLinks(self, log):
        for modelPieceName in self.pieces.keys():
            ...
            if not modelPiece.IsRoot():
                ...
                log.Write(
                    'model-piece "%s" is a child of "%s" (parent has %d children)\n'
    ...
    def SetRadiusAndHeight(self, log):
        ...
        for modelPieceName in self.pieces.keys():
            ...
            log.Write(
                'mid-pos distance of piece "%s": %.2f, {min, max}Y: %.2f, %.2f\n'
...
def SaveSpringOBJMetaDataScript(filename):
    ...
    log.Write(
        '[SpringOBJMetaDataScriptExporter] export log for model "%s"\n\n'
        % (model.GetName())
    )
    ...
    for obj in objects:
        ...
        if (objType != "Mesh") and (objType != "Empty"):
            log.Write(
                'skipping non-mesh object "%s" (type "%s")\n' % (obj.getName(), objType)
            )
            continue
        elif objType == "Mesh":
        ...
        log.Write(
            'processed object "%s" with offsets %s (has parent: %s)\n'
            % (obj.getName(), offsetsStr, isRootStr)
        )

    log.Write(
        "model has %d pieces (has root-piece: %s)\n"
...
Window.FileSelector(
    SaveSpringOBJMetaDataScript, "Export to Spring OBJ meta-data script", "*.lua"
)
